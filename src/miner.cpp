...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SeatherMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SeatherMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SeatherMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("seather-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SeatherMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
