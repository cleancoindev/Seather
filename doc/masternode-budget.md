...
Seather now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<seather_address\> \<monthly_payment_seather\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ STR, _12_ times over the course of a year totaling _24000_ STR.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<seather_address\> \<monthly_payment_seather\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` STR.
 ...
 * vote-many          - Vote on a Seather initiative
 * vote-alias         - Vote on a Seather initiative
 * vote               - Vote on a Seather initiative/budget
